...
REPO_URL = "https://github.com/system-in-miniature/mini-lucene"
META = re.compile(
...
def role_rank(path: str) -> int:
    if path.startswith(("src/minilucene/index/", "src/minilucene/storage/")):
        return 0
    if path.startswith("src/minilucene/"):
        return 1
...
def is_test_evidence(path: str) -> bool:
...
def load_block_layouts(
    ...
    *,
    ...
    expected_failure_files = tuple(
        sorted(path for path in patch_paths if is_test_evidence(path))
    )
    ...
    for index, raw in enumerate(raw_blocks, start=1):
        ...
        test_paths = [item for item in files if is_test_evidence(item)]
        if test_paths:
            raise ValueError(
    ...
    *,
    ...
    while index < len(markers):
        ...
        for path in paths:
            ...
            if is_test_evidence(path) != (region_name == "test contract"):
                raise ValueError(
...
def parse_localized_lesson(
    ...
    *,
    ...
    expected_test_paths = {
        item.path for item in file_patches if is_test_evidence(item.path)
    }
    ...
    for card in cards:
        ...
        chapter_link = (
            ...
            + {
                ...
                2: "02-analysis.md",
                3: "03-inverted-index.md",
                4: "04-codec.md",
                5: "05-segments-nrt.md",
                6: "06-deletes-updates.md",
                7: "07-commit-atomicity.md",
                8: "08-scoring.md",
                9: "09-query-language.md",
                10: "10-merge-and-beyond.md",
                11: "11-daat.md",
            }[card.chapter]
        )
        lines.append(

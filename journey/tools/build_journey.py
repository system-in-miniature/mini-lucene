#!/usr/bin/env python3
"""Build, verify, and prepare MiniLucene Journey workspaces."""
...
def check_chain(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="minilucene-journey-chain-") as raw:
        workspace = Path(raw)
...
def assert_stage_parity(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"minilucene-stage-{stage_number:02d}-"
    ) as raw:
        expected = Path(raw)
...
def main() -> int:
    ...
    default = ROOT.parent / "MiniLucene-journey-workspace"

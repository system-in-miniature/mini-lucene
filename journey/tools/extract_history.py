#!/usr/bin/env python3
"""Resolve MiniLucene's history-driven Journey snapshots from Git evidence."""
...
def patch_for_stage(manifest: HistoryManifest, number: int, *, root: Path) -> bytes:
    ...
    with tempfile.TemporaryDirectory(prefix=f"minilucene-patch-{number:02d}-") as raw:
        repository = Path(raw)
...
def main() -> int:
    ...
    print(f"wrote {len(manifest.stages)} MiniLucene Stage patch/test pairs")
    return 0

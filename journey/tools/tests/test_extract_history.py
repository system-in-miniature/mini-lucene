"""Contracts for history-driven MiniLucene Stage reconstruction."""
...
def test_manifest_is_thirty_contiguous_history_driven_stages() -> None:
    ...
    assert manifest.stages[28].source == "dd4b50c"
    assert manifest.stages[-1].slug == "daat-execution"


def test_parser_regression_replaces_the_earlier_lexer_and_parser() -> None:
    manifest = extract_history.load_manifest(ROOT / "journey" / "manifest.toml")
    ...
    for path in (
        ...
        assert after[path] == extract_history.git_file(ROOT, "dd4b50c", path)

# Stage 18 · Immutable live-doc masks
...
??? note "File diff: tests/storage/test_live_docs_commit.py"
    ```diff
...
??? note "File diff: tests/unit/storage/test_live_docs.py"
    ```diff
...
??? note "File diff: src/minilucene/storage/filesystem.py"
    ```diff

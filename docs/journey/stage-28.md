# Stage 28 · CLI and domain closure
...
??? note "File diff: tests/acceptance/test_end_to_end.py"
    ```diff
...
??? note "File diff: tests/acceptance/test_failure_matrix.py"
    ```diff
...
??? note "File diff: tests/acceptance/test_owner_zero.py"
    ```diff
...
??? note "File diff: tests/contract/test_cli.py"
    ```diff
...
??? note "File diff: src/minilucene/cli.py"
    ```diff
